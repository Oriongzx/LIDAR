...
# 绘制原始和去噪后的平均 k 最近邻距离的散点图
def plot_distances_scatter(original_distances, filtered_distances):
    # 绘制原始和去噪后点云的平均距离分布在同一张图中
    plt.figure(figsize=(8, 6))
    plt.scatter(np.arange(len(original_distances)), original_distances, color='blue', alpha=0.7, label="Original", s=10)
    plt.scatter(np.arange(len(filtered_distances)), filtered_distances, color='green', alpha=0.7, label="Filtered",
                s=10)

    plt.title("Mean K-nearest neighbours distances")
    plt.xlabel("Point Index")
    plt.ylabel("Mean distance to K-nearest neighbours")
    plt.legend(loc="upper right")
    plt.tight_layout()
    plt.savefig("k_nn_distances_comparison.png")  # 可以修改文件名和路径
    plt.show()
...
def main():
    ...
    # 计算原始点云和去噪点云的平均距离
    original_distances = compute_average_distances(cloud, mean_k=50)
    filtered_distances = compute_average_distances(cloud_filtered, mean_k=50)

    # 绘制原始和去噪后的点云的平均 k 最近邻距离的散点图
    plot_distances_scatter(original_distances, filtered_distances)
